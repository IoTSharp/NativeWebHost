...
{
    throw new PlatformNotSupportedException("NativeWebHost.Sample.Mac is intended to run on macOS.");
}

var app = NativeWebApp.CreateBuilder(args)
    .Configure(options =>
    {
        options.Title = "NativeWebHost macOS Sample";
    ...
    {
        options.Title = "NativeWebHost macOS Secondary Window";
...
sealed class SampleApp : IWindowAwareDesktopApp
{
    ...
    public Task OnWindowStartAsync(NativeWebWindowContext window, CancellationToken cancellationToken = default)
    {
        window.Adapter.JsBridge.RegisterHandler("greet", payload =>
        {
            ...
            return Task.FromResult(
                $"\"Hello, {name}! This is macOS window '{window.WindowId}' on {Environment.MachineName}.\"");
        });
    ...
    private static NativeWebWindowDefinition CreateInspectorWindow()
    {
        ...
        {
            Title = "NativeWebHost macOS Inspector Window",
            CustomScheme = "app",
